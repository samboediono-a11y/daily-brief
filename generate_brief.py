...
SECTIONS = [
    ("tech", "TECH", _google_news_url("technology industry", "en-US", "US", "US:en")),
    ("markets", "MARKETS", _google_news_url("stock market", "en-US", "US", "US:en")),
    ("us_news", "US NEWS", "https://news.google.com/rss?hl=en-US&gl=US&ceid=US:en"),
    ("indonesia", "INDONESIA", _google_news_url("Indonesia economy OR Indonesia politics", "en-US", "ID", "ID:en")),
]
...
def call_gemini(candidates_block: str, today: str) -> dict:
    ...
    prompt = PROMPT_TEMPLATE.format(n=STORIES_PER_SECTION, date=today, candidates=candidates_block)
...
def main():
    ...
    (out_dir / f"daily-brief-{today_str}.html").write_text(html, encoding="utf-8")
